...
A page-by-page look at both apps in this project. See the main
[README](../README.md#demo) for how to run either one yourself.

All data shown is synthetic — see the
[README disclaimer](../README.md#disclaimer).

- [Part 1 — Batch dashboard (Streamlit)](#part-1--batch-dashboard-streamlit): 8 pages reading from the dbt marts.
- [Part 2 — Live Operations (React)](#part-2--live-operations-react): 4 pages consuming the real-time order stream.

## Part 1 — Batch dashboard (Streamlit)

Running against the full synthetic year (2025-07-01 → 2026-06-30). See
...
## Part 2 — Live Operations (React)

A second, complementary app (`frontend/`) — real-time, not batch. See
`docs/architecture.md` and `docs/algorithms.md` for how the live order
stream actually works (a Poisson-thinning simulator over a WebSocket,
no message broker).

### 1. Live Operations

Rolling 15-minute KPIs (orders, revenue, avg items/order), a live
orders-per-minute chart, a channel breakdown, and a scrolling order feed
— all updating as events arrive.

![Live Operations](screenshots/live_react_operations.png)

### 2. Order History

Every order seen this session (capped at the most recent 500), with a
sortable table and a channel filter.

![Order History](screenshots/live_react_history.png)

### 3. Session Analytics

Cumulative stats since the server started — total orders, total revenue,
busiest channel, peak orders/minute, and session duration — distinct
from Live Operations' rolling 15-minute window.

![Session Analytics](screenshots/live_react_analytics.png)

### 4. About

Explains the live app's own architecture (the Poisson-process simulator,
why no message broker, why a second app instead of one bigger one) for
anyone clicking through without reading the repo's docs first.

![About](screenshots/live_react_about.png)
